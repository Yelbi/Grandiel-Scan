/**
 * Detección de páginas de un capítulo a partir de la URL base de su carpeta en el CDN.
 *
 * Extraído de app/api/admin/probe-chapter/route.ts para que lo puedan usar tanto
 * la ruta de admin como el sincronizador automático (lib/scraper/sync.ts), que
 * corre en un cron y no puede llamarse a sí mismo por HTTP: /api/admin/* está
 * detrás del Basic Auth del middleware.
 *
 * La lógica de sondeo es la original sin cambios. Lo único que se transformó son
 * los `return NextResponse.json(x)`, que ahora pasan por `probeResult` y devuelven
 * el objeto plano.
 */

export interface ProbeOptions {
  /** URL de la carpeta del capítulo en el CDN. Se le añade "/" final si falta. */
  baseUrl: string;
  /** Extensión de las imágenes. Por defecto 'webp'. */
  ext?: string;
  /** Número de capítulo, usado por los patrones que lo incluyen en el nombre. */
  chapterHint?: number | string | null;
  /** Slug del manga, para patrones tipo "{slug}_01.webp". */
  slugHint?: string;
  /** URL del visor, para CDNs cuyos nombres de archivo llevan hash impredecible. */
  viewerUrl?: string;
}
...
/** Shim de la transformación: conserva las llamadas originales devolviendo el objeto tal cual. */
function probeResult(obj: ProbeResult, _init?: { status?: number }): ProbeResult {
  return obj;
}
...
/* ── Punto de entrada ─────────────────────────────────── */
export async function probeChapter(opts: ProbeOptions): Promise<ProbeResult> {
  const { baseUrl, ext = 'webp', chapterHint, slugHint, viewerUrl } = opts;
  if (!baseUrl) {
    return probeResult({ error: 'baseUrl requerido.' }, { status: 400 });
  }
  if (!/^[a-z0-9]{1,5}$/i.test(ext)) {
    return probeResult({ error: 'Extensión inválida.' }, { status: 400 });
  }
  ...
  if (viewerUrl) {
    for (const candidate of buildViewerCandidates(String(viewerUrl), base)) {
      ...
      if (pages.length > 0) {
        return probeResult({ pages, pattern: 'viewer-page', count: pages.length });
      }
    }
  }
  ...
  if (autoViewerPages.length > 0) {
    return probeResult({ pages: autoViewerPages, pattern: 'auto-viewer', count: autoViewerPages.length });
  }

  // Listado de directorio tiene prioridad: cubre patrones con hash impredecible
  // (ej: "1 - a2404c67.webp", "2 - 4b129cdb.webp" …)
  if (directoryFiles.length > 0) {
    return probeResult({ pages: directoryFiles, pattern: 'directory-listing', count: directoryFiles.length });
  }
  ...
  if (hasSubPad) {
    ...
    return probeResult({ pages, pattern: 'subpart-padded', count: pages.length });
  }

  // Subpart con primera página "-1": 01_01-1.webp, 01_02.webp …
  if (hasSubDash) {
    const pages = await probeSubPartDash(base, ext, session);
    return probeResult({ pages, pattern: 'subpart-dash', count: pages.length });
  }

  // Subpart sin padding: 1_01.webp
  if (hasSubNoPad) {
    const pages = await probeSubPart(base, ext, 1, session);
    return probeResult({ pages, pattern: 'subpart-nopad', count: pages.length });
  }
  ...
  if (hasChapPage4x3) {
    ...
    if (pages.length > 0) {
      return probeResult({ pages, pattern: 'chap4x3(1)', count: pages.length });
    }
  }
  ...
  if (hasCLSubPart) {
    ...
    return probeResult({ pages, pattern: 'cl-subpart', count: pages.length });
  }
  ...
  if (hasZeroParen) {
    ...
    return probeResult({ pages, pattern: 'zero-paren-part', count: pages.length });
  }
  ...
  if (hasParen) {
    ...
    return probeResult({ pages, pattern: 'paren-part', count: pages.length });
  }
  ...
  if (slugHint) {
    ...
    if (prefix) {
      ...
      if (await exists(base + `${prefix}_01.${ext}`, session)) {
        ...
        if (pages.length > 0) {
          return probeResult({ pages, pattern: `slug(${prefix})`, count: pages.length });
        }
      }
      // Página única (sin sufijo numérico): c-463-ingeniero.webp
      if (await exists(base + `${prefix}.${ext}`, session)) {
        return probeResult({ pages: [`${prefix}.${ext}`], pattern: 'slug-single', count: 1 });
      }
    }
  }
  ...
  if (hasDoubleSpaceParen) {
    ...
    return probeResult({ pages, pattern: 'double-space-paren', count: pages.length });
  }
  ...
  if (chapterHint != null && !isNaN(Number(chapterHint))) {
    ...
    if (!hasChapPage4x3 && await exists(base + `${chNumPad}_001.${ext}`, session)) {
      ...
      if (pages.length > 0) {
        return probeResult({ pages, pattern: `chap4x3(${chNum})`, count: pages.length });
      }
    }
    ...
    if (hasChUnpad || hasChPad) {
      ...
      if (pages.length > 0) {
        return probeResult({ pages, pattern: `chapter-subpart(${chNum},pad=${padPart})`, count: pages.length });
      }
    }
    ...
    for (const { prefix, padPart } of prefixVariants) {
      ...
      if (await exists(base + `${prefix}${pp}_01.${ext}`, session)) {
        ...
        if (pages.length > 0) {
          return probeResult({ pages, pattern: `prefixed(${prefix})`, count: pages.length });
        }
      }
    }
  }
  ...
  for (let i = 0; i < prefixes.length; i++) {
    if (pairPrefixResults[i]) {
      ...
      if (pages.length > 0) {
        return probeResult({ pages, pattern: `prefixed-pair(${prefixes[i]})`, count: pages.length });
      }
    }
  }
  ...
  for (let i = 0; i < prefixes.length; i++) {
    if (prefixParenResults[i]) {
      ...
      if (pages.length > 0) {
        return probeResult({ pages, pattern: `prefixed-paren(${prefixes[i]})`, count: pages.length });
      }
    }
  }
  ...
  if (!autoViewerUrl && !simpleConfirmado) {
    ...
    if (contentPages.length > 0) {
      const coverPages = hasZero1 ? await probeSimple(base, ext, 1, 1, 0, session) : [];
      const pages = [...coverPages, ...contentPages];
      return probeResult({ pages, pattern: 'chapter-subpart', count: pages.length });
    }
  }
  ...
  if (hasSimple3) {
    ...
    return probeResult({ pages, pattern: 'simple-3digit', count: pages.length });
  }
  ...
  if (has002 && !hasSimple3) {
    ...
    if (pages.length > 0) {
      return probeResult({ pages, pattern: 'simple-3digit-from-002', count: pages.length });
    }
  }
  ...
  if (has003 && !hasSimple3 && !has002) {
    ...
    if (pages.length > 0) {
      return probeResult({ pages: [...portada, ...pages], pattern: 'simple-3digit-from-003', count: portada.length + pages.length });
    }
  }
  ...
  if (hasSimple2) {
    ...
    return probeResult({ pages, pattern: 'simple-2digit', count: pages.length });
  }
  ...
  if (has02 && !hasSimple2) {
    ...
    if (pages.length > 0) {
      return probeResult({ pages, pattern: 'simple-2digit-from-02', count: pages.length });
    }
  }
  ...
  if (hasZero && hasZero1) {
    ...
    return probeResult({ pages, pattern: 'zero-indexed', count: pages.length });
  }

  // Simple 1 dígito desde 1: 1.webp, 2.webp, 3.webp …
  if (hasZero1) {
    const pages = await probeSimple(base, ext, 1, 1, 2, session);
    return probeResult({ pages, pattern: 'simple-1digit', count: pages.length });
  }
